...
1. Un réseau **U-Net** pour la prédiction spatiale fine.
2. Des blocs **Squeeze-and-Excitation (SE)** intégrés pour modéliser efficacement les interdépendances entre les différentes caractéristiques convolutives.
...
```bash
pip install torch torchvision numpy netCDF4 matplotlib scikit-image tqdm tensorboard jupyter
```

## 📊 Résultats
Comme mis en évidence dans le Rapport Final, STint surpasse significativement les méthodes classiques d'interpolation basées sur le flux optique pour les variables scientifiques, démontrant que les architectures indépendantes du flux (flow-agnostic) sont nettement plus adaptées aux dynamiques spatiales complexes.
